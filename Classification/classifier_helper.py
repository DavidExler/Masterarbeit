...
with open('/Users/davidexler/Documents/Masterarbeit/repo/Masterarbeit/save_data/3D_images_Renamed/masks3D_CELLPOSE_RUN_1.pkl', 'rb') as f:
    masks3D_20xRenamed = pickle.load(f)
blobs_per_image = [len(np.unique(mask)) for mask in masks3D_20xRenamed]

images3D_20xRenamed_full = []
with open('/Users/davidexler/Documents/Masterarbeit/repo/Masterarbeit/save_data/save_data/imgs_20xRenamed.pkl', 'rb') as f:
...
class SAMClassifierMLP(nn.Module):
    def __init__(self, pretrained_ViT, num_classes=5, learnable_upsample=False, D=32):
        super().__init__()
...
class SAMClassifier3D_CENTER_AWARE(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f"[DEBUG] Ecoder-Input shape: {x.shape}") 
        feats = self.encoder(x)  # [B*D, 256, H_out, W_out]
        print(f"[DEBUG] Ecoder-Output shape: {x.shape}") 
        _, C_feat, H_out, W_out = feats.shape
        ...
        x = F.adaptive_avg_pool3d(x, (4, 4, 4))  # [B, 64, 4, 4, 4]
...
def get_resnet101_encoder():
    resnet = models.resnet101(weights=models.ResNet101_Weights.IMAGENET1K_V1)
    encoder = nn.Sequential(*list(resnet.children())[:-2])  # keep conv layers
    encoder = nn.Sequential(
        encoder,
...
def get_efficientnetv2l_encoder():
...
#def get_swinl_encoder():
#    swin = models.swin_v2_b(weights=models.Swin_V2_B_Weights.IMAGENET1K_V1)
#    # swin.features is already sequential, ending with LayerNorm2d(embed_dim)
#    # Grab the final embed dim automatically
#    last_block = list(swin.features.children())[-1]
#    out_channels = 1024  # should be 1024 for swin_v2_b
#
#    encoder = nn.Sequential(
#        swin.features,
...
def get_swinl_encoder():
    swin = models.swin_v2_b(weights=models.Swin_V2_B_Weights.IMAGENET1K_V1)
    encoder = nn.Sequential(
        swin.features,                   # [B, H, W, 1024]
        swin.norm,                        # [B, H, W, 1024]
        Permute(0, 3, 1, 2),               # [B, 1024, H, W]
