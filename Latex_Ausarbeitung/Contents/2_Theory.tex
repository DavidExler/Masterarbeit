...
Ein Problem der Cross-Entropy Loss Minimierung ist ihre Anfälligkeit für Rauschen der \acp{gt}.
...
In Abb. \ref{fig:Segmentation} \cite{kirillov2019PQ} sind beispielhaft \acp{gt} der verschiedenen Arten von Segmentierung zu sehen.
\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.24\textwidth}
...
Es besteht aus einem Bild-Encoder, einem Prompt-Encoder und einem Masken-Decoder (siehe Abb. \ref{fig:SAM_Architektur}). 
\begin{figure}
    \centering
...
\begin{figure}
    ...
    \caption{Ablauf des CellposeSAM-Modells. Eingabebilder werden durch einen Bild-Encoder direkt zu \textit{Flows} umgeformt. Die Gradienten der Flows werden verfolgt und aus dem entstehenden Gradientenfeld werden Segmentierungsinstanzen vorhergesagt \cite{pachitariu2025samcellpose}.}
...
%\textbf{QUELLEN NOCH!}\newline
Für Klassifikatoren werden in der Regel nur Encoder vortrainiert, der Decoder muss an die Klassen der downstream Anwendung angepasst werden \cite{Yosinski2014, Dippel2022, Plested2022}.
State-of-the-Art für Bild-Encodern sind \acp{cnn} oder \acp{vit}, die auf dem ImageNet \cite{Russakovsky2015} Datensatz vortrainiert werden \cite{You2018, Kornblith2019}.
...
Diese residual connections verbinden den Ein- und Ausgang modularer Faltungsschichten (siehe Abb. \ref{fig:ResCon}).
...
Während MBConv aber die Dimension der Daten mithilfe einer weiteren 1x1 Faltung und 3x3 Depthwise-Faltung erhöht, nutzt der Fused-MBConv-Block hierzu eine reguläre 3x3 Faltung (siehe Abb. \ref{fig:MBConv}).\newline
\begin{figure}[htbp]
    \centering
