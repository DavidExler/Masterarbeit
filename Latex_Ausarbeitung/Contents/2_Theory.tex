...
Hierzu werden sowohl die Methoden verwandter Projekte und Studien, als auch die Literatur zum aktuellen \"Stand der Technik\" beleuchtet. Dem theoretischen Hintergrund wird der Neuheitswert der Arbeit gegenübergestellt, um den Beitrag des vorliegenden Projekts zur Forschung zu verdeutlichen.
...
Der State-of-the-Art ist ein Neuronales Netz, das auf Basis der abstrakten Merkmale des Encoder eine Zuversichtlichkeit für jede Klasse ausgibt \cite{Ghods2019}.
Hierzu lernt in der Regel ein Multi-Layer-Perzeptron auf Basis von Trainingsdaten mit zugehöriger \ac{gt} den Zusammenhang zwischen den Merkmalen und der assoziierten Klasse.
Diesen Zus in Gewichtungen von der Ausgabe des Encoders und nicht-linearen Aktivierungsfunktionen auszudrücken \cite{Schmidhuber2015}. \newline
...
Der Decoder muss den Klassen der downstream Anwendung angepasst werden.
...
ResNet ist ein Residual Neural Network, ein \ac{cnn} mit sogenannten residual connections \cite{He2016}.
...
%    \label{fig:ResCon}
%\end{figure}
In ihrem paper stellen die Autoren fünf unterschiedlich tiefe Varianten der ResNet Architektur vor.
...
EfficientNetV2 ist ein Nachfolger der EfficientNet Modellfamilie \cite{Tan2019, Tan2021}.
...
%\begin{figure}
%    \centering
%    \includegraphics[width=0.4\textwidth]{Figures/MBConv.png}
...
\end{figure}
ConvNeXt \cite{Liu2022} ist eine \ac{cnn}-Modellfamilie mit besonders großen Faltungskernen. 
Die ConvNeXt Architektur umfasst fünf modulare Blöcke mit Faltungen und residual connections, wie die ResNet Architektur \cite{He2016}.
Allerdings verändert dabei ConvNeXt einige Details der ResNet Architektur, wie beispielsweise die GeLU Aktivierungsfunktion \cite{Hendrycks2016} und Layer normalization \cite{Ba2016}. 
Swin Transformer \cite{liu2021} sind eine beliebte Modellfamilie von \acp{vit}.
Ihr Nachfolger, die Swin Transformer V2 \cite{Liu2022a} vergrößert die Modelle noch.
...
Zwei Layer normalization \cite{Ba2016} Schichten, ein weiteres MLP-Netz und residual connections vervollständigen anschließend den modularen Swin Transformer V2 Block.
...
\begin{figure}
    ...
    \caption{Swin Transformer V2 Architektur \cite{Liu2022a}. 
