...
\textbf{Bilineare Interpolation} ist ein Verfahren zur Bildnachbearbeitung, das die Dimension eines Bilds erhöht, indem Werte für neue Pixel zwischen bestehenden geschätzt werden \cite{Smith1981}.
...
\textbf{Normierungsmethoden}:\newline
Batch Normalization normalisiert die Eingaben einer Schicht über eine Mini-Batch, indem für jedes abstrakte Merkmal der Schicht, das bei der Dimensionsreduktion des Bilds entsteht, eine Transformation durchgeführt wird \cite{Ioffe2015}.
...
Beide stabilisieren das Training tiefer neuronaler Netze und lassen die Normalisierung als Bestandteil der Modellarchitektur lernen, statt sie nur als Preprocessing-Schritt durchzuführen \cite{Santurkar2018, Xu2019}.\newline
...
%    = \frac{1}{N} \sum_{n=1}^{N} 
%    - \log \Bigg( \sum_{i=1}^{c} 
%    p(\tilde{y} = \tilde{y}_n \mid y = i)\, p(y=i \mid x_n, \theta) \Bigg), 
\end{equation}
...
Eine häufig genutzte Methode ist die Minimierung des Generalized Cross Entropy Loss:
\begin{equation}
%    \argmin_{\theta} 
%    \sum_{i=1}^{n} v_i \mathcal{L}_q(f(x_i; \theta), y_i) - v_i \mathcal{L}_q(k) 
%    = 
    \argmin_{\theta,\, w \in [0,1]^n} 
    \sum_{i=1}^{n} w_i \mathcal{L}_q(f(x_i; \theta), y_i) 
    - \mathcal{L}_q(k) \sum_{i=1}^{n} w_i ,
\end{equation}
für das Klassifikator-Training \cite{Zhang2018}.
\textbf{Genauigkeit}
...
Als Genauigkeit eines trainierten Klassifikators wird dann der maximale Wert bezeichnet, den der Klassifikator während des Trainings erreicht.
Hierzu werden die Modelle nach jeder Epoche evaluiert.
(\textit{Notiz: Ist das hier richtig platziert?})\newline
...
Beispiele für Domänenunterschiede in biomedizinischen Bildaufnahmen sind verschiedene Farb-Marker, Aufnahmegeräte oder Zoom-Stufen (siehe Abb. \ref{fig:domains}). 
Sind verschiedene Darstellungen gleicher Objekte in Bildern wie in Abb. \ref{fig:domains} dargestellt. \newline
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.24\textwidth}
        ...
        \caption{Zieldaten TODO Aufnahmegeräte usw.}
    \end{subfigure} \hfill
    \begin{subfigure}[b]{0.24\textwidth}
...
Ghosh et al. definieren Domänenanpassung: "Gegeben Quell- und Ziel-Domänen $D_s$ und $D_t$ sowie die Aufgaben $\tau_s$ und $\tau_t$, zielen Domänenadaptations-basierte Verfahren darauf ab, ein Modell mit Parametern $\theta$ zu erlernen, das für die Zielaufgabe geeignet ist, wenn $D_s \neq D_t$ und $\tau_s = \tau_t$." \cite{Ghosh2020}.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.24\textwidth}
        ...
        \caption{Panoptisch}
    \end{subfigure}
    \caption{Die verschiedenen Arten der Segmentierung. Links ist das Originalbild zu sehen, rechts folgend sind die zugeordneten pixelweise Labels farblich eingezeichnet. 
...
Als \textbf{Decoder} wird der zusammenfassende Teil des Klassifikators bezeichnet, er gibt einen Zuversichtlichkeitswert für jede Klasse aus.
Der State-of-the-Art für den Decoder ist ein neuronales Netz, das auf Basis der abstrakten Merkmale des Encoders eine Zuversichtlichkeit für jede Klasse ausgibt \cite{Ghods2019}.
...
%\textbf{QUELLEN NOCH!}\newline
Für Klassifikatoren werden in der Regel nur Encoder vortrainiert, der Decoder muss an die Klassen des vorliegenden Problems angepasst werden \cite{Plested2022, Yosinski2014, Dippel2022}.
...
Das verhindert nicht nur das automatische Auslesen der Eigenschaften von Myothuben, sondern unter umständen auch das manuelle Auslesen.
Selbst für Experten sind in dichten Strukturen Instanzen von Myotuben nicht immer eindeutig trennbar.
...
Außerdem gibt es für die speziefischen Aufnahmebedingungen und Marker der vorliegenden Daten keinen angepassten Klassifikator.
Aufgrund der Besonderheiten der vorliegenden Daten ist das Verhalten von vortrainierten Encodern und etablierten Methoden unvorhersehbar.
Eine weitere Fragestellung ist deshlab, wie Methoden der Klassifikation mit der Kombination der Marker umgehen. 
Des Weiteren ist der Umgang mit dreidimensionalen Daten, besonders in Umgebungen ohne große Rechenleistung.
Diverse Lösungen existieren, um dreidimensionale Daten mit Expertenwissen zu versehen, allerdings keine, die angepasst an die vorliegenden Aufnahmen zeitoptimiert ablaufen.
...
Es soll ein optimales Segmentierungsmodell gefunden werden, das die Eigenschaften der Nuclei in den vorliegenden, dreidimensionalen Daten besonders wenig durch Segmentierungsfehler verfälscht.
Das Segmentierungsmodell soll dann genutzt werden, um außerdem einen Ablauf zu schaffen, in dem Experten ihr Wissen zu den Klassen der Nuclei besonders zeiteffizient äußern können.
...
Die enstehenden Annotationen sollen direkt in einen Trainingsablauf für Klassifikatoren eingebunden sein.
Hierbei müssen Klassifikatoren mit dreidimensionalen Daten von variierender Tiefe umgehen können.
Durch einen ausführlichen Methodenvergleich soll durch die Applikaton der vorliegenden Arbeit dann ein Optimum für die Klassifikation der anwesenden Nuclei gefunden werden können.
Dieser Methodenvergleich soll auch für Nutzer*Innen ohne Programmierkenntnisse zugänglich sein.
Außerdem sollen die entstehenden Klassifikatoren einfach ausführbar sein und die ausgelesenen Eigenschaften leicht zugänglich sein.
In einer Parameterbestimmung soll das Optimum explizit für die vorliegenden Aufnahmen durch die vorgestellte Applikation bestimmt werden.
