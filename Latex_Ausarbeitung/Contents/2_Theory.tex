...
Um dieses Problem zu lösen schlägt die Literatur self-attention-Mechanismen vor, die auch nicht-lokale Informationen erfassen \cite{Bertasius2021, Wang2018}. 
%\cite{Carreira2017} - 2D convs auf 3D "aufgeblasen" - nicht gestapelt
%\cite{Tran2015} - 2D convs struggeln in 3D beziehungen Festzustellen
%\cite{Tran2018} - 3D struggles generell, Architektur für 3D mit besserer Z Beziehung
%\cite{Feichtenhofer2020} 2D ist adaptbar auf 3D und wird dadurch bessre, 2D+stitching hat struggels
%\cite{Wang2018} Besonders lange dependencies auf der Z-Ebene sind von 3D-Convs nicht gedeckt- self-attention als Lösung
%Bertasius2021 Attention für space-time zusammenhänge
...
Als Decoder dienen drei Segmentierungsköpfe, die verschiedene Transformationen der gelabelten Trainingsmasken vorhersagen.\\ [0.5\baselineskip]
...
\begin{itemize}
    ...
    Durch das Anreichern dreidimensionaler Daten mittels Segmentierungsmasken und das automatische Fokusieren einzelner Nuclei soll sowohl die Rechenzeit optimiert werden als auch der Aufwand, einzelne Nuclei entlang drei Dimensionen zu suchen, eliminiert werden.
