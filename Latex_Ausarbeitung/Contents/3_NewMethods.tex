...
Es behandelt die selbstentwickelten Beiträge zu den Methoden und die vorliegenden dreidimensionalen Bildstapel der Myotuben-Zellkulturen.
Auf die in Abschnitt \ref{sec:daten} beschriebenen Daten werden die eingeführten Methoden angewandt, um die Effktivität der Methoden zu demonstrieren.
In Abschnitt \ref{sec:Kriterien} wird ein neues Bewertungskriterium für 3D-Instanzsegmentierungsmodelle eingeführt.
Es bewertet Modelle in Hinblick auf ihre Eignung interpretierbare Merkmale von Nuclei unverändert zu extrahieren.
...
Die gesamte Methodik wird in einer modularen Anwendung umgesetzt die 3D-Daten als Eingabe annimmt und interpretierbare Eigenschaften, wie die Verteilung der Klassen und Volumen der anwesenden Nuclei, ausgibt.
Diese Anwendung wird hier 3D-Zelldaten-Pipeline genannt.
...
Zuerst wird dazu das neu eingeführte Bewertungskriterium für Segmentierungsmodelle \acf{ipq} für verschiedene Modelle berechnet (siehe Kapitel \ref{sec:Kriterien}).
...
Die Labeling-App ermöglicht die zeiteffiziente Annotation von Nuclei, indem die relevanten Bildausschnitte automatisch anhand der Segmente extrahiert werden.
...
Die Klassifikatormethoden sind beschrieben in Kapitel \ref{sec:MethodsClassifier} und umfassen 1. Encoder-Architekturen, 2. Klassifikations-Kopf-Architekturen, 3. Vorverarbeitungsmethoden und 4. Vortrainingsmethoden.
Unter den Methoden sind sowohl etablierte, als auch neu entwickelte Ansätze.
Anhand der Genauigkeit der trainierten Klassifikatoren auf einem seperaten Validierungsanteil des Datensatzes werden die Methoden verglichen und eine optimale Konfiguration ausgegeben.
...
Je Kanal ergibt sich eine räumliche Auflösung von 1024 x 1024 Pixeln mit 568nm pro Pixel.
Die Anzahl der Z-Schichten beträgt zwischen 24 und 64.
...
Es besteht aus drei Faktoren, die je eine Fehlerart von Instanzsegmentierungsmasken prüfen.
\begin{figure}[h]
  \centering
  ...
  Außerdem wird ein Fehlerbild durch Berechnung der \acf{iou} hergestellt, hier durch die logische XOR-Fläche der Maske und der Annotation als Platzhalter dargestellt (5).
  Zuletzt wird ein Zuweisungsbild erstellt, das die \acp{tp}, \acp{fp} und \acp{fn} festhält (6).
  Aus diesen drei Teilen wird je ein Faktor für die Berechnung der \ac{ipq} gebildet.
  Durch Vergleiche der Ergebnisse verschiedener Modelle kann dann das optimale Segmentierungsmodell für die Anwendung gewählt werden.}
  \label{fig:ipq}
\end{figure}
\noindent
Als Datensatz wird aus den in Kapitel \ref{sec:bench} vorgestellten Benchmarks der S\_BIAD1518-Datensatz \cite{Kromp2020_Dataset, chen20223_Dataset} genutzt, da dieser nicht in den Trainingsdaten eines zu testenden Segmentierungsnetz vorkommt.
Der Datensatz verfügt über manuelle Annotationen für Instanzsegmentierungsmasken und ist mit synthetisch Generierten Daten und Masken erweitert.
...
Die Aufgabe des Bewertungskriterium ist es, zu quantifizieren, wie gut sich eine vorliegende Instanzsegmentierung eignet, um reale Eigenschaften einer Aufnahme, wie die Zellkernanzahl, Größe der Zellkerne und lokale Zellkerndichten auszuwerten. 
...
Zusätzlich werden durch einen neuen Faktor Verletzungen der injektiven Abbildung von segmentierten Nuclei auf die Instanzen der Annotation negativ bewertet werden, da die genaue Anzahl der Nuclei und die räumliche Dichte der Nuclei eine bedeutungsvolle Metrik für die Auswertung ist. 
...
Jedem instanzsegmentierten Nucleus wird eine Klasse zugewiesen werden, um die Ausgabe zur panoptischen Segmentierungsmaske zu erweitern.
...
Durch diese panoptische Maske und die extrahierten Eigenschaften der Myotuben-Kultur wird ein klarer Überblick über den Status der Zellkultur geboten.\\ [0.5\baselineskip]
Für die Klassenzuweisung ist ein Klassifikator notwendig, der einen Bildausschnitt mit einer Nucleus-Instanz als Eingabe annimmt und eine Klassen als Ausgabe ausgibt.
...
\begin{table}[H]
  ...
  Angegeben sind sowohl die Top-1-Genauigkeit (Acc@1) als auch die Top-5-Genauigkeit (Acc@5), also  ob die korrekte Klasse die zuversichtlichste Vorhersage, bzw. unter den fünf zuversichtlichten Vorhersagen ist.}
...
Swin V2 und der CellposeSAM-Encoder sind \ac{vit}-Architekturen, die anderen Encoder sind \acp{cnn}.
...
Mit der Methode geht der Verlust der Oberflächenmerkmale einher.
Außerdem ist das Klassifikationsergebnis bei dieser Vorverarbeitungsart stärker von der Qualität der Segmentierung abhängig.
Ein Erfolg der Methode weist auf einen hohen Informationsgehalt in der Geometrie hin.\\[0.5\baselineskip]
Die zweite Methode wird hier Distanz-Methode gennant.
...
In Abb. \ref{fig:preproc_methods} sind die Nuclei-Kanäle der verschiedenen Methoden und die Entfernungsmaske dargestellt.
\begin{figure}[H]
  \centering
    \includegraphics[width=0.98\linewidth]{Figures/preproc.pdf}
  \caption{2D-Schnitte von ausgeschnittenen 3D-Bildbereichten zur Darstellung der beiden Vorverarbeitungsmethoden. 
...
Um einen Einfluss auf die Ergebnisse durch ungleiche Verteilung der Annotationen auf die vorhandenen Klassen zu vermeiden, wird vor dem Training die Anzahl der Stichproben pro Klasse extrahiert.
Mithilfe der Verteilung der Anzahlen werden dann die Gradienten stärker gewichtet, die zu unterrepräsentierten Klassen gehören.
...
\begin{figure}[h]
  ...
  Jeder Encoder (linke Seite eines Netzwerks) und jeder Klassifikations-Kopf (rechte Seite eines Netzwerks) wird mit einer dieser drei Label-Mengen trainiert.
  ...
  Anschließend wird mit Expert*Innen-Labels der Klassifikations-Kopf neu trainiert.
...
Das Ziel dabei ist, einen Mittelweg zwischen Informationserhalt und Overfitting-Gefahr sowie Rechenaufwand zu erzielen.
...
Optional werden die Gewichte des Encoder hiernach, bis auf die letzten beiden Schichten eingefroren und nur der Klassifikations-Kopf wird in weiteren 35 Epochen mit dem Trainingsdatensatz der Zieldaten trainiert.
...
Diese Gewichte entstehen durch Vortaining auf dem ImageNet-Datensatz oder stammen aus dem \ac{sam}-Encoder.
Bis auf die letzten 20 bis 30 Prozent der Schichten werden alle Gewichte des Encoders während des Trainings eingefroren.
...
Hier werden zwei neue Klassifikations-Kopf-Architekturen nach dem Vorbild vergleichbarer Anwendungen in der Literatur eingeführt (siehe Abschnitt \ref{sec:Klassifikation}).
...
Er interpretiert die Merkmale, die der Encoder generiert, als Volumen und generiert daraus mithilfe von 3D-Faltungen und Pooling eine neue Repräsentation daraus.
Diese Repräsentation wird dann durch Linear Layers zu vier Ausgabe-Klassen umgeformt.
...
Der \textit{Schichten-Klassifikator} betrachtet die einzelnen Schichten des Bilds anhand der individuellen Schichten, die der Encoder ausgibt mit Hilfe eines self-attention-Mechanismen über die Z-Dimension. 
...
Diese Methode ist dazu da, seperate Instanzen mit der gleichen numerischen Annotation (IDs) zu trennen und mit einzigartigen Zahlenwerten zu versehen. 
...
Eine weitere Methode der Nachbearbeitung, die auf die Segmentierungsmasken eingesetzt werden kann, ist der Watershed-Algorithmus. %(siehe Abschnitt \ref{sec:Segmentierung}).
