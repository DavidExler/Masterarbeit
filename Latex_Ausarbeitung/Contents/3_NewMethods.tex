...
Für das Training wird der Adam Algorithmus \cite{Kinga2015} mit einer Lernrate von 0.0001 eingesetzt.
Außerdem wird der Cross-Entropy-Loss \cite{Sukhbaatar2014} verwendet.  \textbf{ *  muss ich das erklären? (in der theorie dann) *} \newline
Aus den manuell annotierten Bilddaten werden ein Test- und ein Trainingsanteil im Verhältnis eins zu neun extrahiert.
Alle betrachteten Klassifikatoren werden ausschließlich mit den Trainingsdaten trainiert und ihre Leistung ausschließlich mithilfe der Testdaten getestet.
Beide Datensätze werden durch Augmentierung erweitert und in Batches zusammengefasst.
...
Da in vielen Bildausschnitten Nuclei sehr nah aneinander liegen und dem Klassifikator signalisiert werden muss, welcher der sichtbaren Nuclei klassifiziert werden soll, werden für jeden Klassifikator zusätzlich drei verschiedene Eingangsdaten-Formate getestet.
...
Daten des zweiten Typs werden um einen Kanal erweitert, der Segmentierungsmaske enthält.
Alle anderen Kanäle, einschließlich des Nuclei Kanal bleiben unverändert.
Zuletzt werden Daten des Typ drei mit einer Entfernungsmaske skaliert.
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.18\textwidth}
      ...
      \caption{Ausgeschnittener Bereich des Nuclei-Kanals mit mehreren Nuclei. 
      Daten des zweiten Typs beinhalten diese Darstellung.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.18\textwidth}
      \includegraphics[width=\linewidth]{Figures/VERSION1_CH_2_SLICE_4.png}
      \caption{Segmentierungsmaske des zu klassifizierenden Nucleus, wie Daten des zweiten Typs sie enthalten.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.18\textwidth}
      ...
      Diese wird pixelweise mit dem Nuclei-Kanal multipliziert für Typ drei.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.18\textwidth}
      \includegraphics[width=\linewidth]{Figures/VERSION1_CH_0_SLICE_4.png}
      \caption{Darstellung des Typs drei, entstanden durch Multiplikation von Nuclei-Kanal und Entfernungsmaske.}
  \end{subfigure}
...
Eine weitere Methode ist einfache bilineare Interpolation. \textbf{ *  muss ich das erklären? *} \newline
...
Die Encoder der nachfolgenden vortrainierten Netze werden vergleichend eingesetzt.
In der Tabelle sind die Namen, Anzahl der Parameter und, falls vorhanden, die Genauigkeit auf dem ImageNet Datensatz \cite{Russakovsky2015} angegeben.
\begin{table}[ht]
  \centering
