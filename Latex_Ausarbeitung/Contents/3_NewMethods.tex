...
\begin{figure}[h]
  ...
  \caption{Graphical Abstract - Die Abbildung zeigt den Aufbau der Applikation der vorliegenden Arbeit.
  Die Applikation kann regulär angewandt werden (Inferenz) oder optimiert werden (Optimierung).
  Bei der Anwendung werden 3D Daten von der Applikation als Eingabe angenommmen und interpretierbare Eigenschaften wie die Verteilung der Klassen und Volumen der anwesenden Nuclei werden ausgegeben.
  Zur Optimierung werden die 3D Daten auf den Ablauf für den Vergleich der Segmentierungsmodelle oder der Klassifikatormethoden angewandt.
  Für den Vergleich der Segmentierungsmodelle wird mit verschiedenen Modellen das neu eingeführte Bewertungskriterium \acf{ipq} berechnet.
  ...
  Mithilfe der \ac{ipq}-Werte wird ein optimales Modell gewählt.
  Die Architektur und die Parameter des optimalen Model werden in den Ablauf der Applikation eingesetzt.
  ...
  Die Labeling App ermöglicht Experten zeiteffizient Annotationen für die Klassen der Nuclei zu erstellen, indem die relevanten Bildausschnitte anhand der Segmente extrahiert werden.
  Mit den erstellten Annotationen und den 3D Daten werden dann verschiedene Klassifikatoren trainiert.
  Diese Klassifikatoren ergeben sich aus Kombinationen der verfügbaren Klassifikatormethoden.
  Die Klassifikatormethoden umfassen 1. Encoderarchitekturen, 2. Decoderarchitekturen, 3. Vorverarbeitungsmethoden und 4. Vortrainingsmethoden.
  ...
  Die Architektur und die Parameter dieser Konfiguration werden dann in den Ablauf der Applikation eingesetzt.
  }
...
Zuerst wird ein neu entwickeltes Bewertungskriterium für Segmentierungsmodelle beschrieben.
...
Verschiedene Kombinationen der eingeführten Methoden werden beispielhaft auf die Daten der vorliegenden Arbeit angewandt und die entstehenden Klassifikatoren verglichen.
