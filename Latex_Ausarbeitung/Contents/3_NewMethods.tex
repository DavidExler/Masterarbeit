...
\end{equation}
wobei $N$ die Anzahl der Vorhersagen, $\hat{y}_i$ die Vorhersage des Klassifikators und $y_i$ die \ac{gt} ist. 
...
Zum anderen sollen Verletzungen der injektiven Abbildung von segmentierten Nuclei auf die Instanzen der \ac{gt} negativ bewertet werden, da die Anzahl der Nuclei eine bedeutungsvolle Metrik für Nutzer*Innen ist. 
...
\begin{algorithm}
\caption{Beste \ac{gt}-Zuordnung für jede Segmentierungsinstanz}
\begin{algorithmic}
...
\begin{itemize}
    ...
    \item \ac{iou}$(\bigcup_{p_i \in p}p_i, g) \in [0,1] $ die \ac{iou} zwischen allen Segmentierungsinstanzen $p_i$ in der \ac{tp}-Instanz \textit{p} und der zugehörigen \ac{gt}-Instanz \textit{g} beschreibt.   
...
Erst die panoptische Segmentierungsmaske ermöglicht die halbautomatische Überwachung der Entwicklung von Myotubenkulturen.
Nuter*Innen der vorgestellten Methoden wird mit dieser panoptischen Maske und den daraus erzeugbaren, interpretierbaren Merkmalen der Kultur ein klarer Überblick über den Status der Zellkultur geboten.\newline 
...
Mithilfe des Label Spreading-Algorithmus \cite{Zhou2003}, mit einer Radial Basis Funktion \cite{Lowe1988} als Kernelfunktion, werden die Experten-\acp{gt} über die Struktur der Daten auf alle Stichproben ausgebreitet.
Durch den Algorithmus entstehen \acp{gt} für die Daten ohne Experten-Labels.
Diese neuen \acp{gt} werden dann eingesetzt, um in 25 Epochen sowohl die Encoder, als auch die Decoder zu trainieren, mit dem Ziel unter geringem Aufwand für die Experten umfangreiche Klassifikatoren zu trainieren.
