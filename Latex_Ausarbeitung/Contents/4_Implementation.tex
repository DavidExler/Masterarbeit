...
\section{Überblick}
Im nachfolgenden Kapitel wird die Implementierung aller relevanten Methoden erklärt mit Fokus auf die praktische Anwendung dieser Methoden in Form von Nutzerschnittstellen oder als Entwicklerskript.
...
Für das nnU-Net-Modell (siehe Kap. \ref{sec:Seg_Modelle}) wird auf die Eingabedaten zuerst eine Konvertierung angewandt, die die Daten durch Padding in die geforderte Größe gebracht und zu .nii.gz-Dateien umgeformt werden.
...
Außerdem werden neu entwickelte Klassen von PyTorch-Modellen für das Erstellen der Klassifikations-Kopf aufgerufen (siehe Kap. \ref{sec:Decoder}).\\ [0.5\baselineskip]
...
\begin{figure}[H]
    ...
    Als Kästen zu sehen sind die Module, die die 3D-Zelldaten-Pipeline bilden.
...
Vom \ac{gui} gehen Start- und Stop-Signale an die wichtigsten Module aus (Segmentierung, Labeling-App, Visualisierung und Methodenvergleich).
...
Der Datensatz beinhaltet eine Retreiver Instanz (siehe Kap. \ref{sec:Preproc}) und wird von einem Datensatz-Helfer-Modul erstellt.
...
\begin{figure}
    ...
    Über die Bedienelemente wird zwischen Bildern und Zellen umgeschalten, die Klasse des Nucleus bestimmt, der gezeigte Ausschnitt mit prozentualen Schwellenwerten normalisiert und die Fenstergröße geändert.}
...
Des Weiteren ist eine Python-Klasse angelegt, die separat noch das aktuell ausgewählte Bild und den ausgewählten Nucleus speichert. 
Erst wenn eine Änderung der Auswahl ausgeführt wird, wird ein neuer Nucleus oder ein neues gesamtes Bild geladen und selbst dann lediglich aus dem Cache. 
...
\begin{figure}[H]
    ...
    \caption{Auschschnitt des Tabs der 3D-Zelldaten-Pipeline der den Vergleich der Klassifikatormethoden ermöglicht.
...
\begin{figure}[H]
    ...
    \caption{Auschschnitt des Tabs der 3D-Zelldaten-Pipeline, der das finale Training des Klassifikators durchführt.
...
\raggedright Pseudo-Labler & Semi-supervised-Annotationen erstellen & 215 & SciKit-Image, Numy \\[1em]
\raggedright Visualisierungen & Kontur-Bilder, gradCAM, Scatterplots, etc & 650 & SciKit-Image, Numy, Matplotlib, SciPy, Torch \\[1em]
...
\raggedright Websever backend & Unterstützende Funktionen für das Frontend-serving & 100 & - \\[1em]
