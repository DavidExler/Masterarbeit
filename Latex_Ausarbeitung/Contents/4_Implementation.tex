...
\begin{itemize}
    ...
    \item Wie wird die Bewertung durchgeführt?
    \item Wie ist der Workflow für Nutzer*Innen
\end{itemize}
Die Segmentierungsmodelle werden zur Evaluation in einfachen Entwicklerskripts ausgeführt.
...
Eine "Plan" Datei wird nach dem Vorbild des Beispiels der Autoren erstellt, die unter anderem die Anzahl der Wiederholungen, Normalisierungsart und Datensatz Eigenschaften festlegt.
Das Modell wird direkt mit den Gewichten der Veröffentlichung initialisiert und über die Komandozeile auf allen Daten des S-BIAD1518 Datensatz ausgeführt.
Die Ergebnisse werden ebenfalls als .nii.gz Daten gespeichert.
Da die Ergebnisse Instanzen zwar trennt, aber örtlich getrennten Instanzen nicht zwingend einzigartige Label vergibt, wird als Nachbearbeitung wird ein neuer Algorithmus verwendet, der die Labels der Instanzen überarbeitet.
Hierzu werden pro Label einzelne Pixel betrachtet und davon ausgehen alle Pixel mit direkter Verbindung über die Segmentierungsmaske gesucht.
Diese verbundenen Pixel werden als neue einzigartige Instanz abgelegt, bis keine Pixel ohne Verbindung übrig sind. \newline
Für das Deepcell Modell wird direkt das bereitgestellt Jupyter Notebook der Autoren auf die Benchmark Daten angewandt.
Die Ergebnisse werden daraufhin mit dem Watershed Algorithmus der OpenCV Bibliothek nachbearbeitet.\newline
Das CellposeSAM Modell wird als Entwicklerskript weitgehend mit den vorgeschlagenen Parametern der Autoren angewandt, nur der durchschnittliche Durchmesser der Nuclei im Datensatz wird verändert.
Auf die Ergebnisse wird keine Nachbearbeitung angewandt.\newline
Die Ergebnisse von jedem der drei Modelle werden in einem Jupyter Notebook eingelesen und darin durch einen greedy-Algorithmus mit den Instanzen an die Numerierung der Annotationen des S-BIAD1518 Datensatz angepasst.
...
\begin{itemize}
    \item Wie sind die Netze je eingebunden?
    \item Welche Vor-/Nachverarbeitung?
    \item Welche Software Komponenten sind Teil des Trainings?
    \item Wie ist der Workflow für Nutzer*Innen
\end{itemize}
...
Mithilfe der Pytorch Bibliothek werden der entspechende Encoder und, abhängig von der Vortrainingsmethode, entsprechende Gewichte geladen.
Außerdem werden neu entwickelte Klassen von Pythorch-Modellen für das Erstellen der Decoder aufgerufen.
In das zweite Modul wird die Vortrainingsmethode eingegeben und ein entsprechender Datensatz wird zurückgegeben.
Dieser Datensatz wird als Pytorch Dataloader erstellt und mit einer Batch-Göße und mit Augmentierungstransformationen der Monai Bibliothek versehen.
In diesen Dataloader wird mithilfe von zwei neu entwickelten Klassen eine Software-Instanz eingebettet, die es erlaubt die großen, dreidimensionalen Daten nicht alle auf einmal als Variablen im Datensatz unterzubringen, sondern nur die Indices von Bildern und Instanzen.
Diese Indices werden dann genutzt um dynamisch nur die gewünschten Bildausschnitte zu laden. 
...
Das dritte Modul erhählt einen Klassifikatorm und einen Datensatz und führt einen Trainingsdurchlauf durch.
Es speichert die Ergebnisse und die Gewichte der neu trainierten Klassifikatoren in automatisch benannten Dateien.
Im vierten Modul werden mithilfe der plotly Bibliothek Grafiken aus den eingegebenen Ergebnissen erstellt und speichert sie seperat mithilfe der Matplotlib Bilbliothek.

\section{3D-Zelldaten-Pipeline}
Die 3D-Zelldaten-Pipeline vereint die verschiedenen Module der vorliegenden Arbeit.
In den fünf Tabs (Segmentierung, Labeling-App, Methoden Vergleich, Training und Visualisierung) werden die Aufgaben der Anwendung ausgeführt.
...
Zwischenergebnisse wie trainierte Modelle und verarbeitete Daten werden als nicht-interpretierbaren Python-spezifischen Datentypen abgelegt.
...
Im Eingabefeld "Eingabe Ordner" wird ein Ordner, relativ vom Speicherort der Anwendung angegeben, aus dem .TIF Bilder gelesen werden.
...
\subsection{Labeling App}
Um den Klassifikator zu trainieren, werden einigen Zieldaten mithilfe einer selbst erstellten Labeling App Annotationen hinzugefügt.
Die Anforderungen an die Labeling App sind ein nutzerfreundlicher, zeiteffizienter Ablauf und Zugägnlichkeit für Nutzer*Innen ohne Programmiererfahrung.
Funktional muss die Labeling App im Stande sein die zu klassifizierenden Nuclei zu visualisieren und Eingabemöglichkeiten bieten, mit der Experten die Klasse des Nucleus eintragen können.
...
\subsection{Methoden Vergleich}
Der Tab "Methoden Vergleich" ist in Abb. \ref{fig:APP_methods} dargestellt.
...
Die vorangehenden Tabs speichern die Daten direkt im erwarteten Format ab. 
...
Nach Abschluss des Vergleichs werden die Vorraussagen des Modells mit der höchsten Genauigkeit auf dem Validierungsanteil der Eingangsdaten für alle Nuclei im Datensatz abgelegt, auch für die Daten ohne Annotationen.
...
Dabei können auch mehr Annotationen eingesetzt werden, die während dem Methodenvergleich nachgeliefert wurden.
...
\begin{figure}
    ...
    Hier wird ein Ordner gewählt, aus dem die beste Kombination von Methoden gelesen werden soll und anschließend ein längeres Training gestartet.}
    \label{fig:APP_train}
\end{figure}
...
Daraufhin werden unten auf der Seite drei Graphiken dargestellt.
Links wird beispielhaft eine Instanzsegmentierungsmaske von einer Schicht eines der 3D-Bilder gezeigt.
In der Mitte wird ein Balkendiagramm dargestellt, der die Anzahl der Nuclei jeder Klasse angibt.
...
Der verwendete Server verfügt über eine 12th Gen Intel(R) Core(TM) i9-12900KF CPU mit 16 Kernen und 64GB RAM.
