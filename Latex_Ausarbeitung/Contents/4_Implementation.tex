...
Für das nnU-Net-Modell (siehe Kap. \ref{sec:Seg_Modelle}) werden die Eingabedaten zuerst in eine Konvertierung eingegeben wo sie durch Padding in die geforderte Größe gebracht und zu .nii.gz-Dateien umgeformt werden.
...
Für das Deepcell-Modell (siehe Kap. \ref{sec:Seg_Modelle}) wird direkt das bereitgestellte Jupyter-Notebook der Autoren auf die Benchmarkdaten angewandt.
...
Außerdem werden neu entwickelte Klassen von PyTorch-Modellen für das Erstellen der Decoder aufgerufen (siehe Kap. \ref{sec:Decoder}).\newline
...
In diesen Dataloader wird eine neu entwickelte Retreiver-Instanz (siehe Kap. \ref{sec:Preproc}) eingebettet, die es erlaubt, die großen, dreidimensionalen Daten nicht alle auf einmal als Variablen im Datensatz unterzubringen, sondern nur die Indizes von Bildern und Instanzen.
Diese Indizes werden dann genutzt, um dynamisch nur die gewünschten Bildausschnitte auf die GPU zu laden. 
In einem Pseudo-Labler-Modul werden zusätzlich Semi-supervised-Annotationen mithilfe der PCA und der LabelSpreading-Funktion der SciKit-Bibliothek erstellt (siehe Kap. \ref{sec:Pretrain}).\newline
...
\begin{figure}
    ...
    Links werden Daten mit zugehörigen Annotationen und eine Kombination von Methoden eingegeben und Rechts gehen Grafiken, die die Ergebnisse visualisieren ab.
    Verschiedene Python-Bibliotheken werden genutzt (Rosa), aber auch neue Methoden werden eingesetzt (Grün).
    Der Retreiver, der dynamisch Bildausschnitte lädt und der Klassifikator greifen auf die GPU zu (Orange).
    }
...
Oben Zentral zu sehen ist die \acf{gui}, die grafische Schnittstelle mit Bedienelementen und Visualisierungen für Interaktionen mit Nutzer*Innen.
...
\begin{figure}
    ...
    \caption{Signalflussdiagramm der 3D-Zelldaten-Pipeline.
