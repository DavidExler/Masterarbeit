% !TeX root = ../Thesis.tex
%Abschlussvortrag Termin eintragen
%
...
Die Experimente sind die quantitative Bewertungen, die die 3D-Zelldaten-Pipeline durchführt. 
...
Die \ac{ipq}-Ergebnisse werden hier auch in die drei Faktoren der Metrik gegliedert (siehe Kap. \ref{sec:Kriterien}).
Daraus wird ersichtlich, dass jedes der Segmentierungsmodelle in einer der Kategorien dominiert, insgesamt aber CellposeSAM den anderen Modellen signifikant überlegen ist. 
...
Die \ac{ipq} wird auf dem Datensatz mit den Masken von drei vortrainierten Segmentierungsmodellen und, zur Validierung, mit der \ac{gt} ausgeführt.
Die Masken unterscheiden sich optisch stark (siehe Abb. \ref{fig:example_masks}), was sich auch in deutlichen Unterschieden in der \ac{ipq} widerspiegelt.
Abb. \ref{fig:example_masks} zeigt exemplarisch einen 2D-Schnitt eines Bildes mit roten Linien als Konturen der Annotation oder der vorhergesagten Segmentierungsmaske je eines Modells.
\begin{figure}[htbp]
    \centering
    ...
    \begin{subfigure}[t]{0.24\textwidth}
        ...
        \label{fig:gtMaske}
    \end{subfigure}
    \caption{Darstellung der Segmentierungsmasken der verschiedenen Segmentierungsmodelle und der Annotation als Konturen auf einem zweidimensionalen Durchschnitt einer Stichprobe des S\_BIAD1518 Datensatzes.}
...
Außerdem zeigen die Boxplots viele Ausreißer in den Daten, was den Unterschieden zwischen den Bildkategorien, die der Datensatz enthält, geschuldet sein könnte.
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{Figures/Segmentation_Boxplots}
    \caption{Boxplots der Ergebnisse der \ac{ipq}-Berechnungen mit den Faktoren \textit{$k_1$}, \textit{$k_2$} und \textit{$k_3$} jeweils gleich Eins. 
...
Das IQ-Beispiel zeigt die Verletzung injektiver Abbildung der Nuclei auf die Annotation anhand einer nnU-Net-Maske.
Zentral zu sehen ist hier ein nicht-eliptischer Nucleus.
Dieser Nucleus wird vom nnU-Net-Modell in mehrere Segmente unterteilt, wodurch die räumliche Konzentration der Nuclei überschätzt wird.
Nicht-eliptische Nuclei führen häufig zu dieser Art von Fehlern.\newline
Mit dem Fehler gehen hier RQ-Fehler einher, weil zwei der Segmente im Vergleich zur Annotation zu klein sind, um als \ac{tp} erkannt zu werden.
...
\newline\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{Figures/IPQ_examples.pdf}
    \caption{Exemplische Darstellung einzelner Ausprägungen verschiedener Fehlerarten.
    Hierzu sind jeweils, in dieser Reihenfolge, ein Ausschnitt eines 2D-Durchschnitts einer Stichprobe, die Annotation als Kontur und die vorhergesagte Maske eines Modells zu sehen.
    Die erste Zeile zeigt ein Beispiel eines schlechten IQ-Werts anhand einer nnU-Net-Maske.
    In der zweiten Zeile ist ein RQ-Fehler anhand einer CellposeSAM-Maske zu sehen und ganz unten ist ein Beispiel für einen schlechten SQ-Wert anhand einer Deepcell-Maske zu sehen. 
    }
    \label{fig:ipq_examples}
\end{figure}

\section{Klassifikation}\label{sec:resultsClassifier}
Die in Kapitel \ref{ch:NewMethods} vorgestellten, zum Teil neu eingeführten Methoden zur Klassifikation werden durch die neu entwickelte 3D-Zelldaten-Pipeline anhand eines separaten Anteils des manuell gelabelten Datensatzes getestet.
...
Da die Modelle während des Trainings rauschbehaftete Verläufe der Genauigkeit aufweisen und es zu Overfitting kommen kann, wird nicht das Modell am Ende des Trainings zur Evaluation eingesetzt, sondern das Modell der Trainingsepoche, in der die Genauigkeit am höchsten ist.
...
\begin{figure}[h]
    \centering\includegraphics[width=\linewidth]{Figures/Trainingskurven.pdf}
...
ResNet101 und CellposeSAM haben als Encoder die dritt- und zweithöchste durchschnittliche Genauigkeit mit 77,3\% und 77,4\%.
...
\begin{figure}[h]
    \centering\includegraphics[width=\linewidth]{Figures/encoders_compare.pdf}
...
\end{figure}
Um dieses Phänomen zu untersuchen und weitere Auffälligkeiten ausfindig zu machen, werden nachfolgend weitere Vergleiche vorgestellt.
...
\begin{figure}[h]
    \centering\includegraphics[width=\linewidth]{Figures/encodersVdecoder.pdf}
...
\begin{figure}[h]
    \centering\includegraphics[width=\linewidth]{Figures/encodersVchannel1.pdf}
...
\begin{figure}[h]
    \centering\includegraphics[width=\linewidth]{Figures/encodersVpretrain.pdf}
