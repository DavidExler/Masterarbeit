...
Für die Wahl eines Segmentierungsnetzes wurde in Kapitel \ref{ch:NewMethods} das Bewertungskriterien \ac{ipq} eingeführt.
Außerdem wurde in Kapitel \ref{ch:NewMethods} der annotierte S\_BIAD1518 Datensatz vorgestellt.
...
Die Masken unterscheiden sich optisch stark (siehe Fig. \ref{fig:example_masks}), was sich auch in starken Unterschieden in der \ac{ipq} äußert.
\begin{figure}[htbp]
    \centering
...
\end{figure}
\newline
Die Ergebnisse jedes Segmentierungsnetzes sind einzeln für jedes Bild im Appendix \ref{supp:ipq}. 
Eine Zusammenfassung der Ergebnisse ist in den Boxplots in Fig. \ref{fig:boxplots_ipq} gegeben.
Das zentrale Ergebnis ist, dassCellposeSAM die mit Abstand besten \ac{ipq}-Werte erzielt. 
Mit einem Mittelwert von \num{0.64} ist der Wert signifikant höher als der Mittelwert von \ac{ipq} bei nnUNet (\num{0.04}) und Deepcell (\num{0.02}), mit entsprechenden p-Werten von \num{1.80e-80} bzw. \num{1.59e-81} bei einseitigen T-Tests. 
...
Obwohl nnUNet und Deepcell jeweils eine Metrik dominieren wird ihr \ac{ipq}-Wert durch die beiden schlechten Faktoren stark heruntergezogen, während CellposeSAM in jeder Metrik gut, wenn auch nicht am bestern, abschneidet.
...
\newline
\textbf{KOMMENTAR:} Ich weiß nicht, ob der folgende Teil schon als Interpretation/Diskussion zählt, oder generell schon zu viel ist:
\newline
Durch einen Vergleich der Masken mit der \ac{gt} sind die Schwächen und Stärken der einzelnen Netze ersichtlich.
Die nnUNet sind sichtbar kleiner als die Instanzen, was eine schlechte Segmentierungsqualität bedingt.
Oft zerteilen mehrere nnuNet-Masken eine Nuclei-Instanz, was von der Injektiven Qualität gestraft wird.
Wie auch die Recognition Qualität zeigt findet hierbei nnUNet sehr zuverlässig die anwesenden Nuclei mit mindestens einer Maske.
Deepcell (siehe Fig. \ref{fig:DeepcellMaske}) übersegmentiert die Nuclei, wodurch die Segmentierungsqualität stark abnimmt.
Das Ergebnis sind Masken, die zu groß sind und oft mehr als einen Nucleus enthalten.
Durch diese Übersegmentierung wird vermieden, dass Instanzen der \ac{gt} durch die Deepcell-Masken geteilt werden, was zu der guten Injektiven Qualität führt.
\begin{figure}[t]
    \centering
