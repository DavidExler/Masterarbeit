...
Die Instanzen können in der Nachbearbeitung auch durch eine Dilatation vergrößert werden.
Das erhöht vermutlich die \ac{sq}, ändert aber nicht nichts an der schwachen \ac{iq}.\\ [0.5\baselineskip] 
...
Die Bilddomäne sowie Eigenschaften wie Größe, Exzentrizität und Rundheit der Zieldaten unterscheiden sich von denen des Benchmark.
Da keine Annotationen für die Instanzsegmentierung der Zieldaten verfügbar sind, wird von der Anwendung der Methoden im Zuge dieser Arbeit abgesehen, um kein Overfitting auf den Benchmark-Datensatz zu riskieren. \\[0.5\baselineskip]
...
\end{itemize}
In der Praxis gehen die Fehler oft mit Fehlern einer anderen Art einher, was daran liegt, dass die Modelle nicht demselben Denkverhalten folgen wie menschliche Betrachter*Innen.
...
Da ResNet18 der genauste Encoder ist, liegt dennoch die Vermutung nahe, dass noch kleinere Encoder die Genauigkeit weiter erhöhen können.\\[0.5\baselineskip]
...
Links ist ein Beispiel besonders starker Überschneidung gegeben.
...
\begin{figure}[H]
    ...
    \caption{Beispiele für eine starke und eine schwache Überschneidung von Nuclei nach Anwendung der Distanzmethode.
...
Obwohl die Bilddomäne biologischer Zelldaten stark von den Bildern des ImageNet-Datensatzes abweicht, werden bedeutsame Bildmerkmale extrahiert.
...
Der Swin V2-Encoder hat ohne Vortraining nur 50\% Genauigkeit erreicht, was vermutlich daran liegt, dass die aufwendige Transformer-Architektur nicht ausreichend mit dem geringen Trainingsumfang der Zieldaten lernen kann.
...
\begin{figure}[H]
    ...
    \caption{Genauigkeiten der verschiedenen Vortrainingsmethoden abhängig von der Parameteranzahl des genutzen Encoders.}
...
Die durchschnittliche Genauigkeit des Swin V2-Encoder ImageNet-Vortraining ist 44\%, mit ImageNet-Vortraining ist sie bei 84 \%. 
...
Die Genauigkeit der meisten Encoder ist auch ohne ImageNet-Vortaining annehmbar, vor allem, da mithilfe der 3D-Zelldaten-Pipeline auch ein optimaler Encoder passend zur Vortrainingsmethode gewählt wird.
...
Ein weiterer Ausblick für die Arbeit ist die Erweiterung der Instanzsegmentierung auf den Myotuben-Kanal.
In einem ersten Versuch wurde bereits getestet, wie die panoptische Segmentierung der Nuclei für die Segmentierung der Myotuben genutzt werden kann.
Da die Geometrie der Myotuben-Zellkerne meist eine lange Achse aufweist, können diese Nuclei als als Linie geschätzt werden.
Nuclei mit ähnlicher Ausrichtung der Langen Achse, die annähernd auf einer Geraden entlang dieser Achse liegen, werden verbunden.
Die entstehenden Linien werden dem \ac{sam}-Modell als Hinweise gegeben und in 2D segmentiert.
Bisher sind sowohl der Algorithmus zu Linienbildung und auch die Segmentierung noch sehr ausbaufähig, kommende Forschung kann diesen Ansatz aber noch weiter verfolgen.